...
/**
 * Orders 4 arbitrary 2D points into Top-Left, Top-Right, Bottom-Right, Bottom-Left
 */
export function orderQuadCorners(points: Point2D[]): [Point2D, Point2D, Point2D, Point2D] {
  ...
  // Classify points by angle or relative position
  ...
  const tl = sorted[minSumIndex];
  const tr = sorted[(minSumIndex + 1) % 4];
  const br = sorted[(minSumIndex + 2) % 4];
  const bl = sorted[(minSumIndex + 3) % 4];
...
/**
 * Calculates destination width and height for warped rectangle
 */
export function calculateWarpDimensions(corners: [Point2D, Point2D, Point2D, Point2D]): {
  ...
  const widthTop = euclideanDistance(tl, tr);
  const widthBottom = euclideanDistance(bl, br);
...
/**
 * Estimates initial 4 document corners with safe inset margin
 */
export function detectDefaultCorners(imgWidth: number, imgHeight: number): [Point2D, Point2D, Point2D, Point2D] {
  ...
  return [
    ...
    { x: imgWidth - marginX, y: marginY }, // Top-Right
    { x: imgWidth - marginX, y: imgHeight - marginY }, // Bottom-Right
    { x: marginX, y: imgHeight - marginY }, // Bottom-Left
  ];
}

/**
 * Applies perspective warp transformation from source quad corners onto orthogonal destination canvas
 */
export function warpPerspectiveCanvas(
  ...
  // We can draw triangular subdivisions to simulate homography warp smoothly in HTML5 Canvas
  const [p0, p1, p2, p3] = corners; // TL, TR, BR, BL

  // Subdivide quad into mesh grid of cells for perspective accuracy
  const GRID_SIZE = 16;
  for (let y = 0; y < GRID_SIZE; y++) {
    for (let x = 0; x < GRID_SIZE; x++) {
      ...
      // Source quadrilateral interpolation
      ...
      // Destination orthogonal coordinates
...
/**
 * Applies color/B&W scan filtering and image adjustments
 */
export function applyScanFilters(
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    if (options.filter === 'bw') {
      // Thresholding for clean text on white paper
      ...
      // Enhanced color: auto-boost saturation and contrast
      ...
      // Original with optional brightness/contrast
      if (brightness !== 0 || options.contrast !== 0) {
