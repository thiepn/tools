...
function parseIpv4(address:string){const parts=address.split('.').map(Number);return parts.length===4&&parts.every(x=>Number.isInteger(x)&&x>=0&&x<=255)?parts:null}
function ipv4Private(address:string){const parts=parseIpv4(address);if(!parts)return false;return parts[0]===10||parts[0]===127||(parts[0]===169&&parts[1]===254)||(parts[0]===172&&parts[1]>=16&&parts[1]<=31)||(parts[0]===192&&parts[1]===168)}
...
export function parseIceCandidate(candidate:string):IceCandidateInfo{
 ...
 if(/\.local$/i.test(address)){family='mdns';scope='mdns'}else if(parseIpv4(address)){family='ipv4';scope=ipv4Private(address)?'private':'public'}else if(address.includes(':')){family='ipv6';scope=ipv6Private(address)?'private':'public'}
