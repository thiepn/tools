...
export function repeatText(input:string,options:RepeatOptions):string{
  ...
  const prefix=options.prefix??'',suffix=options.suffix??'',separator=options.separator??'',start=Math.floor(options.startNumber??1);
  const numberingChars=options.numberLines&&count>0?Array.from({length:count},(_,i)=>String(start+i).length+2).reduce((a,b)=>a+b,0):0;
  const estimated=count*(input.length+prefix.length+suffix.length)+Math.max(0,count-1)*separator.length+numberingChars;
  if(estimated>OUTPUT_LIMIT)throw new Error('Result would exceed the 2,000,000-character safety limit. Reduce repeats, separators, prefixes, suffixes, or input size.');
  return Array.from({length:count},(_,i)=>`${prefix}${options.numberLines?`${start+i}. `:''}${input}${suffix}`).join(separator);
}
...
export function wrapText(input:string,options:WrapOptions):string{
  ...
  if(output.length>OUTPUT_LIMIT)throw new Error('Wrapped result exceeds the 2,000,000-character safety limit.');
  return output;
}
function hardWrap(text:string,width:number,first:string,rest:string){const out:string[]=[];const units=graphemes(text);let offset=0,indent=first;while(offset<units.length){const available=Math.max(1,width-graphemes(indent).length);out.push(indent+units.slice(offset,offset+available).join(''));offset+=available;indent=rest;}return out.join('\n');}
...
export function generateLorem(options:LoremOptions):string{
  ...
  if(output.length>OUTPUT_LIMIT)throw new Error('Generated result exceeds the 2,000,000-character safety limit.');
  return output;
}
