...
describe('P5 public image catalog',()=>{
  it('contains 19 unique image routes after format-converter consolidation',()=>{expect(PUBLIC_IMAGE_TASKS).toHaveLength(19);expect(new Set(PUBLIC_IMAGE_TASKS.map(t=>t.id)).size).toBe(19);expect(PUBLIC_IMAGE_TASKS.some(t=>t.id==='image-converter')).toBe(true);for(const task of PUBLIC_IMAGE_TASKS)expect(TOOLS_REGISTRY.some(t=>t.id===task.id&&t.category==='image')).toBe(true);});
  it('keeps browser capability claims truthful',()=>{const converter=PUBLIC_IMAGE_TASKS.find(t=>t.id==='image-converter')!;expect(converter.description).toMatch(/browser-decodable|supported locally/i);const upscale=PUBLIC_IMAGE_TASKS.find(t=>t.id==='image-upscaler')!;expect(upscale.description).toContain('high-quality browser resampling');expect(upscale.description).toContain('without claiming AI-generated detail');const privacy=PUBLIC_IMAGE_TASKS.find(t=>t.id==='privacy-blur-image')!;expect(privacy.description).toContain('Manually');});
