...
describe('P13 Office and eBook interchange',()=>{
 it('retains 14 unique P13 routes in the expanded catalog',()=>{expect(PUBLIC_P13_TASKS).toHaveLength(14);expect(new Set(PUBLIC_P13_TASKS.map(t=>t.id)).size).toBe(14);for(const task of PUBLIC_P13_TASKS)expect(TOOLS_REGISTRY.some(t=>t.id===task.id)).toBe(true)});
