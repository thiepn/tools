...
describe('R17 strict A-to-S uplift',()=>{
 ...
 it('stress-tests browser-realistic empty, zero, negative and extreme finite states safely',()=>{for(const id of STRICT_A_TO_S_TARGETS){for(const input of[{text:'',a:0,b:0,c:0},{text:'Ω\u200B👩‍🚀',a:-1,b:-250,c:-5},{text:'x'.repeat(500),a:1e9,b:1e6,c:999}]){const output=analyzeStrictA(id,input).map(x=>String(x.value)).join(' ');expect(output,id).not.toMatch(/\b(?:NaN|undefined|Infinity|\[object Object\])\b/)}}});
