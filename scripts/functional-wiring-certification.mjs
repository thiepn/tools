...
async function makeFixtures(){
  ...
  if(spawnSync('ffmpeg',['-version'],{stdio:'ignore'}).status===0){f.video=path.join(dir,'sample.webm');const p=spawnSync('ffmpeg',['-loglevel','error','-f','lavfi','-i','color=c=blue:s=160x90:d=0.6','-f','lavfi','-i','sine=frequency=440:duration=0.6','-shortest','-c:v','libvpx-vp9','-c:a','libopus','-y',f.video],{stdio:'ignore'});if(p.status!==0)delete f.video}
  return f;
}
  ...
  try{
    ...
    for(let i=0;i<rows.length;i++){
      ...
      try{
        ...
        if(row.category==='device'||row.category==='pdf'){
          result.mode='delegated-browser-certification';
        }else{
