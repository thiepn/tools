...
for (const required of [oldAssertion, oldToolIds]) {
  if (!baselineSource.includes(required)) throw new Error('R5 baseline structure changed; review the expansion wrapper before continuing.');
}

const patchedSource = baselineSource
...
try {
  ...
  const result = spawnSync(process.execPath, [TEMP_SCRIPT], { cwd: ROOT, stdio: 'inherit', env: process.env });
